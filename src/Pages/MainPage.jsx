...
const questions = [
  ...
  {
    question: "What is the best Coder in Uniques?",
        ...
        <main className="w-[22rem] rounded-3xl shadow-xl h-[35rem] bg-white">
          ...
          <div className="w-full flex justify-center mt-10">
            <div className="w-[90%] space-y-6 text-center">
              <p className="text-xl font-medium text-gray-700">Correct: <span className="text-green-600">{quizResults.correct}</span></p>
              <p className="text-xl font-medium text-gray-700">Incorrect: <span className="text-red-600">{quizResults.incorrect}</span></p>
              <p className="text-xl font-medium text-gray-700">Unattempted: <span className="text-yellow-600">{quizResults.unattempted}</span></p>
            </div>
          </div>
        </main>
      </div>
    ...
    <div className="w-full h-screen flex items-center justify-center font-[Chillax] bg-[#f0f0f0]">
      <main className="w-[22rem] rounded-3xl shadow-xl h-[35rem] bg-white">
        <div className="flex flex-col items-center w-full bg-[#026670] rounded-t-3xl py-5">
          ...
          <div className="w-[90%] flex justify-start mt-7">
            <p className="text-2xl font-semibold text-white">Question {currentQuestionIndex + 1} / {questions.length}</p>
          </div>
        </div>
            ...
            <div className="w-[90%] flex justify-between text-lg">
              ...
              <button
                className={`px-4 rounded text-white ${
                  selectedOption ? 'bg-[#026670] hover:bg-[#024b5a]' : 'bg-gray-400 cursor-not-allowed'
                } transition-all`}
                onClick={selectedOption ? handleSubmit : handleNextQuestion}
                disabled={!selectedOption && currentQuestionIndex === questions.length - 1}
              >
                {selectedOption ? 'Submit' : 'Next'}
              </button>
            </div>
          </div>
        </div>
